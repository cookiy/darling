window.onload = function(){
	...
	window.EmployeeView = Backbone.View.extend({
		tagName : 'tr',
		//template中使用的方法_.template($('#item-template').html())是前面提到的underscore.js中提供一个工具方法
		//可以通过界面的HTML模板和一个JSON生成动态的HTML
		//说白了就是把JSON里面的值填充到HTML模板中对应的占位符里面去，牛X的是HTML模板里面支持一些常用的逻辑表达式如if,else,foreach等
