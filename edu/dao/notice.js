...
exports.queryTodayBulletin = function( date , callback){
    mysql.query("select id,content,create_time from bulletin where id = (select min(id) from bulletin where date(create_time) = ?)",[date], function(err, bulletins) {
...
exports.queryLeftDefaultBulletin = function( size, callback){
    mysql.query("select count(*) counts,date(create_time) dates,weekday(date(create_time)) weeks,content from bulletin group by date(create_time) order by id desc  limit 0,? ",[size], function(err, bulletins) {
        callback(err, bulletins);
    });
};
/**
 * 公告总数
 */
exports.queryCountBulletin = function( callback){
    mysql.query("select count(*) counts from bulletin  ", function(err, bulletins) {
        callback(err, bulletins);
    });
};
/**
...
exports.queryPageBulletin = function( index , size, callback){
    mysql.query("select id,content,create_time from bulletin order by id desc limit ?,? ",[index,size], function(err, bulletins) {
        callback(err, bulletins);
    });
};
/**
...
exports.queryLeftPageBulletin = function( index , size, callback){
    mysql.query("select count(*) counts,date(create_time) dates,weekday(date(create_time)) weeks,content from bulletin group by date(create_time) order by id desc limit ?,? ",[size,index,size], function(err, bulletins) {
        callback(err, bulletins);
    });
};
/**
 * 目标日期当天全部记录
 */
exports.queryPageDateBulletin = function( date ,callback){
    mysql.query("select id,content,create_time from bulletin where date(create_time) = ? order by id desc  ",[date], function(err, bulletins) {
        callback(err, bulletins);
    });
};
/**
