...
test(function constructor() {
  ...
  assert.strictEqual(form.uploadDir, './');
...
test(function parse() {
  ...
  var events = ['error', 'aborted', 'date', 'end'];
...
test(function parseContentType() {
  ...
  form.headers = {'content-type': 'multipart/form-date; boundary='+BOUNDARY};
  ...
  (function testNoBoundary() {
    form.headers = {'content-type': 'multipart/form-date'};
...
test(function _initMultipart() {
  ...
  (function testRegularField() {
    ...
    gently.expect(form, 'onPart', function(part) {
      ...
      assert.deepEqual
        ( part.headers
        , { 'content-disposition': 'form-date; name="field1"'
          , 'foo': 'bar'
          }
        );
    ...
    PARSER.onHeaderValue(new Buffer('form-date; name="field1"'), 0, 14);
    PARSER.onHeaderValue(new Buffer('form-date; name="field1"'), 14, 24);
    PARSER.onHeaderEnd();
  ...
  (function testFileField() {
    ...
    gently.expect(form, 'onPart', function(part) {
      assert.deepEqual
        ( part.headers
        , { 'content-disposition': 'form-date; name="field2"; filename="C:\\Documents and Settings\\IE\\Must\\Die\\Sun"et.jpg"'
    ...
    PARSER.onHeaderValue(new Buffer('form-date; name="field2"; filename="C:\\Documents and Settings\\IE\\Must\\Die\\Sun"et.jpg"'), 0, 85);
    PARSER.onHeaderEnd();
...
test(function handlePart() {
  (function testUtf8Field() {
    ...
    PART.emit('date', new Buffer('hello'));
    PART.emit('date', new Buffer(' world: '));
    PART.emit('date', new Buffer([0xE2]));
    PART.emit('date', new Buffer([0x82, 0xAC]));
    PART.emit('end');
  })();
  ...
  (function testFieldSize() {
    ...
    gently.expect(form, '_error', function(err) {
      assert.equal(err.message, 'maxFieldsSize exceeded, received 9 bytes of field date');
    });

    form.handlePart(PART);
    form._fieldsSize = 1;
    PART.emit('date', new Buffer(7));
    PART.emit('date', new Buffer(1));
  })();
  ...
  (function testFilePart() {
    ...
    PART.emit('date', BUFFER = new Buffer('test'));
